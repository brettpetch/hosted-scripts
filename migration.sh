...
function set_ini() {
  ...
  echo "Setting ${section} ${param} in ${file} to ${newval}"
...
EOF
  ...
  for i in $HOME/.install/.*.lock; do
    ...
    totalsize=$(rsync -ah "$HOME/" "${newuser}@${newhost}:old/" -e 'ssh -p22' --dry-run --stats | awk 'END{print $4}')
     echo "Total size of data to transfer: $totalsize"

  if [[ -z $slackWebhookUrl ]]; then
    # Fire transfer starting to ticket
    jsonInit='{"blocks":[{"type":"section","text":{"type":"mrkdwn","text":"Starting data transfer:\n*<https://my.hostingby.design/admin/supporttickets.php?action=viewticket&id='$ticket'|Ticket - #'$ticket'>*"}},{"type":"section","fields":[{"type":"mrkdwn","text":"*Source:*\n'"$HOSTNAME:$HOME"'"},{"type":"mrkdwn","text":"*Destination:*\n'"$newhost:/home/$newuser"'"},{"type":"mrkdwn","text":"*Data:*\n'$totalsize'"}]}]}'
    curl -X POST -H 'Content-type: application/json' --data "$jsonInit" $slackWebhookUrl &>/dev/null
  fi

  echo "Copying data from $HOME to ${newhost}:${newuser}/old... This could take some time."
  rsync -ahH --info=progress2 "$HOME/" "${newuser}@${newhost}:old/" -e 'ssh -p22'

  if [[ -z $slackWebhookUrl ]]; then
    # Fire transfer finished to ticket
    jsonCompleted='{"blocks":[{"type":"section","text":{"type":"mrkdwn","text":"Completed data transfer:\n*<https://my.hostingby.design/admin/supporttickets.php?action=viewticket&id='$ticket'|Ticket - #'$ticket'>*"}},{"type":"section","fields":[{"type":"mrkdwn","text":"*Source:*\n'"$HOSTNAME:$HOME"'"},{"type":"mrkdwn","text":"*Destination:*\n'"$newhost:/home/$newuser"'"},{"type":"mrkdwn","text":"*Data:*\n'$totalsize'"}]}]}'
    curl -X POST -H 'Content-type: application/json' --data "$jsonCompleted" $slackWebhookUrl &>/dev/null
  fi

}

function runner() {
  if [[ ! -f $HOME/old/.info.lock ]]; then
...
function migratePlex() {
  ...
  echo "Setting variables from plex configs"
  ...
  echo "Changing custom connections from ${old_plex_custom_connections}:${old_plex_port} to ${plex_custom_connections}:${plex_port}"
  ...
  echo "Copying data over to new plex install"
...
function migrateQbit() {
  ...
  downloadMigraterr
  "$HOME/.local/bin/migraterr" bencode edit --glob "$HOME/old/.local/share/qBittorrent/BT_backup/*.fastresume" --replace="${oldhome}/|${HOME}/"
...
function downloadMigraterr() {
  if [[ -f $HOME/.local/bin/migraterr ]]; then
    echo "migraterr was previously installed... Skipping!"
    return
  fi
  mkdir -p "$HOME/.tmp/"
  echo "Installing migraterr to edit fastresume data. This will be accessible after as \`migraterr\`"
  version=$(github_latest_version zze0s/migraterr)
  curl -sL "https://github.com/zze0s/migraterr/releases/download/${version}/migraterr_${version//v/}_linux_x86_64.tar.gz" -o "$HOME/.tmp/migraterr.tar.gz"
  cd "$HOME/.tmp/" || {
    echo "Failed to download migraterr" && exit 1
  }
  tar xvf "$HOME/.tmp/migraterr.tar.gz"
  rm -f migraterr.tar.gz README.md
  chmod +x "$HOME/.tmp/migraterr"
  mkdir -p "$HOME/.local/bin"
  mv "$HOME/.tmp/migraterr" "$HOME/.local/bin/migraterr"
}
...
function migrateRtorrent() {
  ...
  downloadMigraterr
  oldhome=$(get_ini "$HOME/old/.info.lock" "general" "HOME")
  "$HOME/.local/bin/migraterr" bencode edit --glob "$HOME/old/.sessions/*.torrent.rtorrent" --replace="${oldhome}/|${HOME}/"
